...
class Request
{
    ...
    /** @var BillingAgreementCreateRequest */
    protected $_billingAgreementCreateRequest;
     ...
     */
    public function createRequest($billingToken, $paypalCMID)
    {
        $this->_billingAgreementCreateRequest = $this->_paypalApi->getAgreementCreateRequest($billingToken);
        $this->_billingAgreementCreateRequest->prefer('return=representation');

        $requestBody = $this->buildRequestBody();

        if ($paypalCMID) {
            $this->_billingAgreementCreateRequest->headers[self::PAYPAL_CLIENT_METADATA_ID_HEADER] = $billingToken;
        }

        $this->_billingAgreementCreateRequest->body = $requestBody;

        try {
            $this->_eventManager->dispatch('paypalcp_create_billing_agreement_before', ['paypalCMID' => $paypalCMID, 'quote' => $this->_quote, 'customer' => $this->_customer]);

            /** @var \PayPalHttp\HttpResponse $response */
            $response = $this->_paypalApi->execute($this->_billingAgreementCreateRequest);

            $this->_eventManager->dispatch('paypalcp_create_billing_agreement_after', ['quote' => $this->_quote, 'paypalResponse' => $response]);
        } catch (\Exception $e) {
